...
function deObjetoAarray(objeto) {
   ...
   // Tu código:
   const arrayDeArray = Object.entries(objeto);
...
function capToFront(string) {
   ...
   for (let i = 0; i < separar.length; i++) {
      if (separar[i] === separar[i].toUpperCase()) {
         mayusculas += separar[i];
      }else{
         minusculas += separar[i];
      }
   }
...
function asAmirror(frase) {
   ...
   const nuevoString = palabras.map(palabra => palabra.split('').reverse().join('')).join(' ');
   return nuevoString;
}
...
function sortArray(arrayOfStrings) {
   ...
   // Tu código:
   const sort = arrayOfStrings.sort(function(a,b){
